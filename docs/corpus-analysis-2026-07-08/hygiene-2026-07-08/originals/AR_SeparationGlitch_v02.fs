...
uniform float voronoiScale1;
uniform float voronoiScale2;
uniform float voronoiOffset1;
uniform float voronoiOffset2;
uniform float grayR;
uniform float grayG;
uniform float grayB;
uniform float easeExp1;
uniform float easeExp2;
uniform float smoothMin;
uniform float smoothMid;
...
void main(){
    ...
    vec4 orig = texture(sTD2DInputs[0], p);
    ...
    // ► sample displaced
    vec4 c1 = texture(sTD2DInputs[0], d1);
    vec4 c2 = texture(sTD2DInputs[0], d2);
